...
if (isset($_POST['servicio']) && isset($_POST['mesa']))
{
		...
		else
		{
	        ...
	        for ($i = 0; $i < count($lstProd); $i++) 
	        {
	        	...
	        	if ($lstCant[$i] > 0)
	        	{
					...
					$query .= " (servicio,numero,producto,cantidad, precio,comentario) ";
					$query .= " VALUES ($servicio,$comanda,'$producto',$cantidad,$precio,'$comentario')";
	        ...
	        {
	        	// Variables para el Error
	 		    $titulo      = "Error en Alta de Servicio";
				$descripcion = "[".$query."]".$conexion->error;
				$enlace      = "atencion.php";
